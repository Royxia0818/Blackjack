...
if __name__ == '__main__':
    ...
    for i in range(agent_num):
        for j in range(agent_num):
            ...
            score_logger.write(
                f'智能体 {agent_names[i]:20} v.s. {agent_names[j]:20}\n')
            score_logger.write('-'*60 + '\n')
            score_logger.write(
                f'智能体 {agent_names[i]:20} 胜率 | {win_rate:.1%}\n')
            score_logger.write(f'{"总对局":29} | {num_round}\n')
            score_logger.write(
                f'智能体 {agent_names[i]:20} 主场 | 胜 {win_hc:3} | 平 {draw_hc:3} | 负 {num_round//2-win_hc-draw_hc:3}\n')
            score_logger.write(
                f'智能体 {agent_names[i]:20} 客场 | 胜 {win_vc:3} | 平 {draw_vc:3} | 负 {num_round//2-win_vc-draw_vc:3}\n')
            score_logger.write('-'*60 + '\n')
            score_logger.write(
                f'智能体 {agent_names[j]:20} 胜率 | {(num_round-win_hc-draw_hc-win_vc-draw_vc)/num_round:.1%}\n')
            score_logger.write(f'{"总对局":29} | {num_round}\n')
            score_logger.write(
                f'智能体 {agent_names[j]:20} 主场 | 胜 {num_round//2-win_hc-draw_hc:3} | 平 {draw_hc:3} | 负 {win_hc:3}\n')
            score_logger.write(
                f'智能体 {agent_names[j]:20} 客场 | 胜 {num_round//2-win_vc-draw_vc:3} | 平 {draw_vc:3} | 负 {win_vc:3}\n')
            score_logger.write('='*60 + '\n')
            score_logger.write('\n')
