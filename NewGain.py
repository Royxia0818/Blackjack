...
class Player:
    ...
    def updateq(self, obs, action, G):
        ...
        ha = int(have_A)
        '''
        self.c[action, ha, dealer_card, sum_hand] += 1
        self.a[action, ha, dealer_card, sum_hand] += G
        self.q[action, ha, dealer_card, sum_hand] = self.a[action, ha,
                                                           dealer_card, sum_hand]/self.c[action, ha, dealer_card, sum_hand]
        '''
        ...
        if action == 1:
            if have_A:
                ...
                maxq = np.max(self.q[:, 1, dealer_card, possible_state])

                not_have_A_possible_state = [
                    sum_hand+_-10 for _ in range(1, 11) if sum_hand+_ > 21]
                nmaxq = np.max(
                    self.q[:, 0, dealer_card, not_have_A_possible_state])
                if nmaxq > maxq:
                    maxq = nmaxq
                self.q[1, 1, dealer_card, sum_hand] += alpha * \
                    (reward+gamma*maxq-self.q[1, 1, dealer_card, sum_hand])
            else:
                possible_state = [
                    sum_hand+_ for _ in range(1, 11) if sum_hand+_ <= 21]
                maxq = np.max(self.q[:, 0, dealer_card, possible_state])

                if sum_hand+11 <= 21:
                    maxq_A = np.max(self.q[:, 1, dealer_card, sum_hand+11])
                    if maxq < maxq_A:
                        maxq = maxq_A
                self.q[1, 0, dealer_card, sum_hand] += alpha * \
                    (reward+gamma*maxq-self.q[1, 0, dealer_card, sum_hand])
        else:
            if have_A:
                self.q[0, 1, dealer_card, sum_hand] += alpha * \
                    (reward-self.q[0, 1, dealer_card, sum_hand])
            else:
                self.q[0, 0, dealer_card, sum_hand] += alpha * \
                    (reward-self.q[0, 0, dealer_card, sum_hand])
        '''
...
def main():
    ...
    np.save("ndealerQ", dealer.q)
    print(f'p_player: \n{player.q[1]-dealer.q[0]}')
    np.save("nplayerQ", player.q)
